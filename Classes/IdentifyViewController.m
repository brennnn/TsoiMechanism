...
NSArray *explosionMagentaFrames;
double iPadScale;
...
- (void)viewDidLoad
{
	...
	NSMutableArray *tempMagentaImages = [[NSMutableArray alloc] init];
	...
	for (int i=0; i <= 15; i++)
	{
		...
		[tempMagentaImages addObject:[UIImage imageNamed:[NSString stringWithFormat:@"explode_red%d.png", i]]];
	...
	explosionMagentaFrames = (NSArray*) tempMagentaImages;
	...
	switch (draggingType) // based on the dragging type, do the following on the release of a finger:
	{
		...
		case NUCLEOSONIC: // If the Np was being dragged
			...
			else
			{
				...
				[Animations fireLaser:laser fromCannon:cannon toPoint:touchPoint];
				[Animations createExplosionInView:self.view atPoint:touchPoint withImages:explosionMagentaFrames];
